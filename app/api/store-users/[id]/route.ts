...
async function putHandler(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const storeUser = await StoreUser.findById(params.id);
    if (!storeUser) {
    ...
    const updatedStoreUser = await StoreUser.findByIdAndUpdate(params.id, body, { new: true });
...
async function deleteHandler(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await StoreUser.findByIdAndDelete(params.id);
